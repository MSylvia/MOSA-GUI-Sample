...
namespace MOSA1.Driver
{
    public static class PS2Keyboard
    {
        ...
        public static void OnInterrupt()
        {
            if (b)
            {
                KData[0] = IOPort.In8(Port_KeyData);
            }
            else
            {
                KData[1] = IOPort.In8(Port_KeyData);

                KeyAvlb = true;

                if (KData[1] == (byte)KeyCode.CapsLock)
                {
        ...
        public static string ToStr(this KeyCode keyCode)
        {
            switch (keyCode)
            {
                case KeyCode.A:
