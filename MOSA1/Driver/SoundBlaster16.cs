using Mosa.Kernel.x86;
using Mosa.Runtime;
using Mosa.Runtime.x86;
using System;

namespace MOSA1.Driver
{
    class SoundBlaster16
    {

        public const ushort DSP_Mixer_port = 0x224;
        public const ushort DSP_Mixer_data_port = 0x225;
        public const ushort DSP_Reset = 0x226;
        public const ushort DSP_Read = 0x22A;
        public const ushort DSP_Write = 0x22C;
        public const ushort DSP_Read_Status = 0x22E;
        public const ushort DSP_16bit_Interrupt_Acknowledge = 0x22F;

        public const ushort Set_time_constant = 0x40;
        public const ushort Set_Output_Sample_Rate = 0x41;
        public const byte Turn_speaker_on = 0xD1;
        public const ushort Turn_speaker_off = 0xD3;
        public const ushort Stop_playing_8_bit_channel = 0xD0;
        public const ushort Resume_playback_of_8_bit_channel = 0xD4;
        public const ushort Stop_playing_16_bit_channel = 0xD5;
        public const ushort Resume_playback_of_16_bit_channel = 0xD6;
        public const ushort Get_DSP_version = 0xE1;

        public const byte Master_volume = 0x22;
        public const byte Set_IRQ = 0x80;

        public static void Initialize()
        {
            Native.Out8(DSP_Reset, 1);
            Native.Hlt();
            Native.Out8(DSP_Reset, 0);

            byte R = Native.In8(DSP_Read);

            if (R == 0xAA)
            {
                Boot.Console.WriteLine("Sound Blaster 16 Found");
            }
            else
            {
                Boot.Console.WriteLine("Sound Blaster 16 Not Found");
            }

            //Turn Speaker ON
            IOPort.Out8(DSP_Write, Turn_speaker_on);
            SetSoundVolume(0xF);
        }

        public static void SetSoundVolume(byte v)
        {
            IOPort.Out8(DSP_Mixer_port, Master_volume);
            //Max
            IOPort.Out8(DSP_Mixer_data_port, v);
        }

        public static byte[] Error = new byte[] {  };

        public static void Play(byte[] Data)
        {
            uint a = (uint)Intrinsic.GetObjectAddress(Data);

            //DMA Channel 1
            IOPort.Out8(0x0A, 5);
            IOPort.Out8(0x0C, 1);
            IOPort.Out8(0x0B, 0x49);
            IOPort.Out8(0x83, (byte)(a >> 16 & 0xFF));
            IOPort.Out8(0x02, (byte)(a & 0xFF));
            IOPort.Out8(0x02, (byte)(a >> 8 & 0xFF));
            IOPort.Out8(0x03, (byte)(Data.Length & 0xFF));
            IOPort.Out8(0x03, (byte)(Data.Length >> 8 & 0xFF));
            IOPort.Out8(0x0A, 1);

            //program sound blaster 16
            IOPort.Out8(DSP_Write, 0x40);
            IOPort.Out8(DSP_Write, 64);
            IOPort.Out8(DSP_Write, 0xC0);
            IOPort.Out8(DSP_Write, 0x00);

            IOPort.Out8(DSP_Write, (byte)(Data.Length & 0xFF));
            IOPort.Out8(DSP_Write, (byte)(Data.Length >> 8 & 0xFF));
        }
    }
}
